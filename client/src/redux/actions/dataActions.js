...
export const fetchAddress = (userData, history) => (dispatch) => {
  ...
  axiosNewInstance
    ...
    .then((result) => {
      const formattedAddress = "bangalore";//result.data.results[0].formatted_address;
      const lat = "1";//result.data.results[0].geometry.location.lat;
      const lng = "1";//result.data.results[0].geometry.location.lng;
      userData.lat = lat;
      userData.lng = lng;
